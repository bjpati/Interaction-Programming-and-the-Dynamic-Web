...
function compareIngredientsCB(ingredientA, ingredientB){
    ...
    //if aisles are the same we compare them by name
    else{
...
function compareDishTypeCB(dishA, dishB){
    ...
    const dishTypeRankingA = dishType(dishA);
    const dishTypeRankingB = dishType(dishB);

    if (dishTypeRanking[dishTypeRankingA] > dishTypeRanking[dishTypeRankingB]){
        return 1;
    }
    else if (dishTypeRanking[dishTypeRankingA] < dishTypeRanking[dishTypeRankingB]){
        return -1; 
    }
    else {
        return 0; 
...
function menuPrice(dishesArray){
    ...
    const sum = arrayOfdishesArrays.reduce(dishesSumCB, 0);
